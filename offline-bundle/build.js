...
function isJSFile(file) {
  ...
  // Also, some other files are including Math.seedrandom code which is breaking embeddable.html.
  // Again, JSmol seems to work fine without those files (even though their names sound serious).
  if (file.path.indexOf('core/corejmol.js') !== -1) return;
  if (file.path.indexOf('core/corejsv.js') !== -1) return;
  if (file.path.indexOf('core/corejmoljsv.js') !== -1) return;
...
// Copy additional files used by JSmol.
fs.copySync(path.join(mainDir, 'jsmol/j2s/img'), path.join(outputDir, 'jsmol/j2s/img'));
